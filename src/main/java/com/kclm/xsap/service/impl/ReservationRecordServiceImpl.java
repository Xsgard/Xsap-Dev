...
public class ReservationRecordServiceImpl extends ServiceImpl<ReservationRecordDao, ReservationRecordEntity> implements ReservationRecordService {
    ...
    public void addReserve(ReservationRecordEntity reservationRecord) {
        ...
        //验证卡类型是否满足
        checkIsMeetCardType(reservationRecord, courseEntity);
        //会员卡信息校验
        cardValidation(valDay, memberBindRecord, courseEntity);
        //判断会员是否满足课程要求
        checkIsMeetCourseRequire(courseEntity, member);
        ...
        //判断课程是否已经开始
        checkCourseIsStart(now, startTime, globalSet, previousDateTime);
        //预约截止时间校验
        checkReserveIsEnd(globalSet, startTime, now);
        //是否预约过该课程
        ...
        checkReservations(one, courseEntity);
        //课堂容量是否足够
        ...
        if (!saved)
            throw new BusinessException(saveFail);
        //查询课程记录信息
